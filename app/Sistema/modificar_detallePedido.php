...
$usuario_consulta = mysql_query("SELECT cod_producto, nombre_producto, precio_unitario, n_equipo FROM producto WHERE cod_producto=".$codigo_producto ) or die(header ("Location:error.php"));
...
if($necesita_equipo == "No")
{
	...
	echo'<form action="modificar_detallePedido2.php?id_cliente='.$codigo_cliente.'&id_pedido='.$codigo_pedido.'&id_producto='.$cod_producto.'&precio='.$precio_unitario.'&id_dp='.$codigo_dp.'&id_pagina='.$pagina.'" method="post"><table width="40%" align="center" border="0" cellpadding="0" cellspacing="0">
  <tr> 
  ...
  <tr> 
    ...
    <td class="campotablas3" width="50%" ><input type="text" name="nombres"  maxlength="3" tabindex="5" class="Formulario" value="';
	$usuario_consulta2 = mysql_query("SELECT cantidad FROM detalle_pedido WHERE cod_dp=".$codigo_dp ) or die(header ("Location:error.php"));

	$registro2= sacar_registro_bd($usuario_consulta2);

	echo $registro2['cantidad'];
