...
-->
 ...
 <?PHP
	...
	$id_dp=$_GET['id'];
	$id_cp=$_GET['id_producto'];
	
	require_once("manejomysql.php");
	conectar_bd();
	$consulta="SELECT codigo_producto, nombre_producto, precio_unitario_prod, stock, marca, industria, stock_minimo, observaciones_producto, imagen1, preferencial, regular, irregular from producto where codigo_producto='$id_cp'";
	//echo $consulta;
...
<form action="mod_producto2.php?cod='.$registro['codigo_producto'].'&id_cliente='.$id_cliente.'&id_pedido='.$id_pedido.'&id_dp='.$id_dp.'" method="post" name="ventas" onSubmit="return validarFormulario(this);">
...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFCC" bgcolor="#E4E4E4" style="TABLE-LAYOUT: fixed">
			...
			<tr> 
              <td height="27" class="campotablas">Cantidad actual</td>
              <td class="campotablas">'.$cantidad.'</td>
              <td class="campotablas">Modificar Cantidad de productos:</td>
              <td class="campotablas"><input type="text" name="cantidad" maxlength="20" tabindex="2" class="Formulario" value='.$cantidad.'> </td>
