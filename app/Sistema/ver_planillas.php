...
$id_cliente=$_GET['fecha'];
list($a3,$m,$d2)=split("-", $id_cliente);
if($m == 12)
{
			$f1=$a3."-".$m."-01";
			$a3=$a3+1;
...
//echo "select sum(monto_salario) as p from planillas where fecha_planilla=".$_GET['fecha'];
$monto_consulta = mysql_query("select sum(monto_salario) as p from planilla where fecha_planilla='".$_GET['fecha']."'");
$monto_t=mysql_fetch_array($monto_consulta);

echo '<br>
<table width="45%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr> 
    <td colspan="2" class="title"><font color="#000033">Resumen Planilla de Pago</font></td>
  </tr>
  <tr> 
    <td width="43%" class="campotablas" >Planilla de Pagos correspondiente al mes de:</td>
    <td width="57%" class="campotablas">Desde  '.$f1.' hasta '.$f2.'</td>
  </tr>
  <tr>
    <td class="campotablas">Monto Total Planilla de pagos en Bs</td>
    <td class="campotablas">&nbsp;'.$monto_t['p'].'</td>
  </tr>
</table>
...
for ( $j=0; $j< cuantos_registros_bd($usuario_consulta); $j++)
{
		...
		echo '<br> <font color="#660000" size="3">Empleado Nr.'.$nume.'</font>';
...
<table width="46%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr> 
    ...
    <td width="27%" class="campotablas">&nbsp;'.$a['nombre_emp']." ".$a['apellido_emp'].'</td>
  </tr>
  <tr> 
    <td class="campotablas">Cargo:</td>
    <td class="campotablas">&nbsp;'.$b['cargo'].'</td>
  </tr>
  <tr> 
